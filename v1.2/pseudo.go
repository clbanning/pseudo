...
// See: http://riot.ieor.berkeley.edu/Applications/Pseudoflow/maxflow.html for information on C source.
// Also, Robbie Wright's discussion here: https://groups.google.com/forum/#!searchin/golang-nuts/hochbaum/golang-nuts/irDjLVYto-Q/11TVvgbXAwAJ.
...
// Setting PseudoCtx.DisplayCut == true either directly or through a config file
...
func displayCut() []string {
	...
	for i := uint(0); i < numNodes; i++ {
		if adjacencyList[i].label >= gap {
			s = append(s, fmt.Sprintf("n %d", adjacencyList[i].number))
		}
	}
...
func result(header string) []string {
	...
	// add cut nodes
	if PseudoCtx.DisplayCut {
		ret = append(ret, "c ")
		ret = append(ret, displayCut()...)
	}
...
// be "c Data: <input>".
func Run(input string, header ...string) ([]string, error) {
	var fh *os.File
	var err error
