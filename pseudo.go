...
func (s *Session) checkOptimality() []string {
...
func (s *Session) displayCut() []string {
	var gap uint
	if s.ctx.LowestLabel {
		gap = s.lowestStrongLabel
	} else {
	...
	for i := uint(0); i < s.numNodes; i++ {
		if s.adjacencyList[i].label >= gap {
			ret = append(ret, fmt.Sprintf("n %d", s.adjacencyList[i].number))
		}
	}
...
func (s *Session) displayFlow() []string {
	var ret []string
	for i := uint(0); i < s.numArcs; i++ {
		ret = append(ret,
			fmt.Sprintf("f %d %d %d", s.arcList[i].from.number, s.arcList[i].to.number, s.arcList[i].flow))
	}
...
//	f 1 3 10
//	...
func (s *Session) result(header string) []string {
	...
	// add Solution
	ret = append(ret, s.checkOptimality()...)

	// add cut nodes
	if s.ctx.DisplayCut {
		ret = append(ret, "c ")
		ret = append(ret, s.displayCut()...)
	}

	// add flows
	if !s.ctx.DisplayCut {
		ret = append(ret, "c ", "c SRC DST FLOW")
		ret = append(ret, s.displayFlow()...)
	}
...
func (s *Session) printArcList() {
	fmt.Println("n from to flow capacity direction")
	for n, v := range s.arcList {
		if v == nil {
			fmt.Println("nil")
			continue
		}
...
func (s *Session) printStrongRoot(n uint) {
	if n >= uint(len(s.strongRoots)) {
...
func (s *Session) printStrongRoots() {
	fmt.Println("n: start end")
	for n, v := range s.strongRoots {
		if v == nil {
			fmt.Println("nil")
			continue
		}
